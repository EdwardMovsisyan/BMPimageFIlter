...
Matrix Webcam::GetMatrixFromFrame(const Webcam::RGBImage& frame)
{
    Matrix matrix;
	...
	for (int i = 0; i < matrix.Height; ++i) 
	{
		...
		{
			int pos = (i * matrix.Width + j) * 3;
			    Pixel pixel;
			    pixel.R = (unsigned char) frame.data[pos + 0];  ///red
			    pixel.G = (unsigned char) frame.data[pos + 1];  ///red
			    pixel.B = (unsigned char) frame.data[pos + 2];  ///red

			//matrix.Data[i][matrix.Height - j - 1] = pixel; Here
			//matrix.Data[i][j] = pixel;
			matrix.Data[i][j] = pixel;
		}
	}
	...
	const char* imageFileName = fileName.c_str(); // (char*) "bitmapImage.bmp";

	int i, j;
	for (i = 0; i < height; i++) {
		for (j = 0; j < width; j++) {
			int pos = (i * width + j) * 3;
			image[i][j][2] = (unsigned char) frame.data[pos + 0];  ///red
			image[i][j][1] = (unsigned char) frame.data[pos + 1];  ///green
			image[i][j][0] = (unsigned char) frame.data[pos + 2];  ///blue
		}
	}*/
...
Matrix Webcam::Frame(int timeout)
{
    for (;;) {
        fd_set fds;
        struct timeval tv;
        ...
        if (read_frame()) {
            //return rgb_frame;
            return GetMatrixFromFrame(rgb_frame);
        }
...
void Webcam::init_device(void)
{
    ...
    if (force_format) {
        ...
        fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; // V4L2_PIX_FMT_RGB32; // V4L2_PIX_FMT_YUYV;
